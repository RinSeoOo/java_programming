...
public class concertReserv {
        ...
        for(int k=0;k<3;k++){
            for(int i=0;i<10;i++){
                if(alph[k][i] == this.name){
                    System.out.println(this.name+" reservation is exist. And "+i+"is "+alph[classNum-1][i]);
                    tf = false;
                    break;
                }
                else {
                    //System.out.println(this.name+" reservation isn't exist");
                    tf = true;
                    break;
                }
            }
    ...
    public void reserv(boolean tf, int num){
        ...
        for(int i=0;i<10;i++){
            ...
            try {
                n = Integer.valueOf(alph[classNum-1][i]);
            } catch (NumberFormatException e){
                n = -1; // 숫자로 변환할수 없는 문자열 형태이면 -1으로 초기화
            }
